...
* 1.4 如果你的网站特殊， 那么请自定义解析函数， 并配置正确
* 1.5 执行work_spider.py脚本， 开始抓取，检测，入库
* 1.6 执行proxy_api.py脚本， 开启API服务
* 1.7 执行delete_not_update_ip.py脚本， 对超过存活时间阀值的IP进行重新检测， 删除或更新插入时间
********
...
start_check     
开始检测函数， 其中用户检测的函数为_request.py中的valid函数， 因为要符合框架的要求，所以自定义的这个函数需要返回两个值，在这里我自定义的返回了响应时间与检测的IP    
save_ip     
...
> 包括的方法:     
valid\[检测函数\];    
因为要符合框架的要求，所以自定义的这个函数需要返回两个值: 响应时间与检测的IP. 
********
* **proxy_basic_config.py**     
`代理网页设置脚本`        
> 包括的字段:    
...
(2) value    
 ...
 \[2] request_method    
 请求方法， 当为post的时候， 必须定义提交的post_data， 否则会报错.因项目的特殊性，如post里面带着网页，所以萎了获得
  多页的数据， 这里就需要定义多个post_data数据，所以在这里将post_data 定义为列表， 里面的数据为字典格式.默认为get， 非必须.     
  \[3] url    
   代理网址， 列表类型， 把需要爬取的页面都放在里面. 必须.   
  \[4] parse_type    
    解析类型， 默认提供: xpath， re. 如果你需要别的解析方式， 那么请定义自己的parse_func.默认的解析函数即上文中的\[parse_to_get_ip]
  只提供xpath与re两种方式.当使用默认解析器的时候， 必须.    
  \[5] ip_port_together    
  ip和port是否在一个字段中， 若是则设置为True， 不是则设置为False， 当使用re进行解析时非必须.    
  ...
  \[6] parse_method    
  解析的具体路径， 上一条有了部分的解释， 当使用xpath与re时必须.    
  当通过re进行解析时， 若解析的类型为re， 则ip_port_together可以为任意的值，parse_method中只有一个键: _pattern    
  \[7] parse_func    
